...
async def settings(message: types.Message):
    ...
    await message.bot.send_message(text=f"{SpecialSymbols.BULLET_MARK.value} Будьласка оберіть мову:",
...
async def select_language(callback_data: types.CallbackQuery, state=FSMContext):
    ...
    await callback_data.message.edit_text(text=f"{SpecialSymbols.BULLET_MARK.value} "
